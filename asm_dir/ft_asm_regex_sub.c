...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/08 19:10:06 by abodnar           #+#    #+#             */
/*   Updated: 2018/05/08 19:10:07 by abodnar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_asm_abodnar.h"

t_handle_func	ft_asm_regex_sub_after_label(char **str, int *res,
													t_asm_valid *params)
{
	int		i;
	int		cmd_name_len;
	...
	cmd_name_len = (int)ft_strlen(g_op_tab[4].name);
	if (ft_strnequ(*str, g_op_tab[4].name, (size_t)cmd_name_len))
	{
		i = -1;
...
t_handle_func	ft_asm_regex_sub_save_start_label(char **str, int *res,
															t_asm_valid *params)
{
	int		label_len;
	char	*new_label;
	...
	if (ft_asm_regex_is_duplicated_label(new_label, params))
	{
		ft_strdel(&new_label);
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_sub_after_label);
	}
...
t_handle_func	ft_asm_regex_sub_start(char **str, int *res,
												t_asm_valid *params)
{
	char	*label_end;
	...
	if ((label_end = ft_strchr(*str, LABEL_CHAR)) != NULL
		...
		return ((void*)ft_asm_regex_sub_save_start_label);
	return ((void*)ft_asm_regex_sub_after_label);
}
...
int				ft_asm_regex_sub(char *champ, t_asm_valid *params)
{
	...
	(void)params;
	func_curr = (t_handle_func)ft_asm_regex_sub_start;
	res = 1;
