...
t_handle_func	ft_asm_regex_el_save_start_label(char **str, int *res,
														t_asm_valid *params)
{
	int		label_len;
	char	*new_label;
...
t_handle_func	ft_asm_regex_el_start(char **str, int *res,
										t_asm_valid *params)
{
	char	*label_end;
...
int				ft_asm_regex_empty_line(char *champ, t_asm_valid *params)
{
	...
	(void)params;
	func_curr = (t_handle_func)ft_asm_regex_el_start;
	res = 1;
