												...
												t_asm_valid *params)
{
		...
		&& label_end < ft_strchr(*str, '\n'))
	{
		...
		{
			if (ft_strchr(*str, COMMENT_CHAR) > label_end)
				if (ft_strchr(LABEL_CHARS, *(label_end - 1)))
					return ((void*)ft_asm_regex_fork_save_start_label);
			return ((void*)ft_asm_regex_fork_after_label);
		}
		if (ft_strchr(*str, COMMENT_CHAR_ADD))
		{
			if (ft_strchr(*str, COMMENT_CHAR_ADD) > label_end)
				if (ft_strchr(LABEL_CHARS, *(label_end - 1)))
					return ((void*)ft_asm_regex_fork_save_start_label);
			return ((void*)ft_asm_regex_fork_after_label);
		}
